...
import { Eye, EyeOff, Loader2, LogIn, UserX } from "lucide-react";
...
export function CheckoutAuth({ onAuthSuccess, onGuestCheckout }: CheckoutAuthProps) {
  ...
  const handleGuestCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Failed to proceed as guest");
    } finally {
  ...
  return (
    ...
    >
      <Tabs value={checkoutMode} onValueChange={(v) => setCheckoutMode(v as "account" | "guest")} className="w-full">
        ...
        <TabsContent value="account" className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Log in or create an account to track your order and save your preferences.
          </p>
        ...
        <TabsContent value="guest" className="space-y-4">
          <div className="text-center py-2">
            <UserX className="w-10 h-10 mx-auto text-muted-foreground mb-3" />
            <h4 className="font-medium mb-1">Continue as Guest</h4>
            <p className="text-sm text-muted-foreground">
              Just enter your email for order confirmation. No account required!
            </p>
          </div>
          ...
          <form onSubmit={handleGuestCheckout} className="space-y-4">
            ...
            <Button type="submit" variant="gradient" className="w-full" disabled={guestLoading}>
              {guestLoading ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
                "Continue as Guest"
          ...
          <div className="bg-accent/30 rounded-lg p-3 text-sm">
            <p className="text-muted-foreground">
              💡 <strong>Tip:</strong> Create an account to track orders, save favorites, and get exclusive offers!
            </p>
          </div>
        </TabsContent>
      </Tabs>
